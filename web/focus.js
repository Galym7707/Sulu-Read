...
function reviewReading(spokenTokens, targets) {
  ...
  for (let ti = 1; ti <= targetCount; ti++) {
    ...
    for (let ki = 1; ki <= tokenCount; ki++) {
      ...
      // Numerals are compared by value, off the engine's best guess only. An alternative cannot
      // help here the way it helps a word: the value comparison already crosses the digits/words
      // divide that a spelling comparison could not.
      if (isDigits(normalizeForMatch(target)) || isDigits(normalizeForMatch(token.text))) {
        for (let targetSpan = 1; targetSpan <= Math.min(MAX_NUMERAL_SPAN, ti); targetSpan++) {
          for (let tokenSpan = 1; tokenSpan <= Math.min(MAX_NUMERAL_SPAN, ki); tokenSpan++) {
            ...
            const tokenRun = spokenTokens.slice(ki - tokenSpan, ki).map((t) => t.text);
            const hasDigitSide =
  ...
  while (ti > 0) {
    ...
    if (move >= MOVE_NUMERAL_BASE) {
      ...
      const heard = spokenTokens.slice(ki - tokenSpan, ki).map((t) => t.text).join(" ");
